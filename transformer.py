...
lm_heads = torch.ones(hidden_dim, vocab_size//TP, device=my_device, dtype=type)
...
if my_rank == root_rank:
    ...
    print(f"lm_heads [d, k/TP]: {lm_heads.shape}, elements: {lm_heads.nelement()}, size: {lm_heads.element_size() * lm_heads.nelement() / 1e9:.2f} GB")
