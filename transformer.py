...
Q = torch.ones(num_layers, hidden_dim, hidden_dim//TP, device=my_device, dtype=type)
K = torch.ones(num_layers, hidden_dim, hidden_dim//TP, device=my_device, dtype=type)
V = torch.ones(num_layers, hidden_dim, hidden_dim//TP, device=my_device, dtype=type)
...
q_ = torch.empty(num_heads//TP//DP, seq_length, hidden_dim//num_heads, device=my_device, dtype=type)
k_ = torch.empty(num_heads//TP//DP, seq_length, hidden_dim//num_heads, device=my_device, dtype=type)
v_ = torch.empty(num_heads//TP//DP, seq_length, hidden_dim//num_heads, device=my_device, dtype=type)
...
if my_rank == root_rank:
    ...
    print(f"q_ [h/TP/DP, N, d/h]: {q_.shape}, elements: {q_.nelement()}, size: {q_.element_size() * q_.nelement() / 1e6:.2f} MB")
    print(f"k_ [h/TP/DP, N, d/h]: {k_.shape}, elements: {k_.nelement()}, size: {k_.element_size() * k_.nelement() / 1e6:.2f} MB")
    print(f"v_ [h/TP/DP, N, d/h]: {v_.shape}, elements: {v_.nelement()}, size: {v_.element_size() * v_.nelement() / 1e6:.2f} MB")
