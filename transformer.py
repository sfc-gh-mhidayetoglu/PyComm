...
def attention_2D(input, Q, K, V, O, c, c_, attention, group_TP, group_DP) -> torch.Tensor:
    ...
    q_ = torch.empty(DP, hidden_dim//TP//DP, seq_length//DP, device=my_device, dtype=type)
    k_ = torch.empty_like(q_)
    v_ = torch.empty_like(q_)
    ...
    q_ = torch.transpose(q_, 0, 2)
    k_ = torch.transpose(k_, 0, 2)
    v_ = torch.transpose(v_, 0, 2)
