...
for i in range(0, 3000):
    ...
    if my_rank == root_rank:
        print(f"iter {i} {data[i]} elements perf {perf_time*1e6:.2f} event {event_time*1e3:.2f} perf max {time_max*1e6:.2f} event max {event_max*1e3:.2f} ms throughput: {bytes / event_time / 1e9:.2f} GB/s")
