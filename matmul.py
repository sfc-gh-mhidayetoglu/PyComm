...
event_comm_p2p_start = torch.cuda.Event(enable_timing=True)
event_comm_p2p_end = torch.cuda.Event(enable_timing=True)
...
event_comm2_p2p_start = torch.cuda.Event(enable_timing=True)
event_comm2_p2p_end = torch.cuda.Event(enable_timing=True)
...
def matmul_2D(hidden_dim = 16384, batch_size = 1024, num_layers = 126, TP=8, DP = 2, mini_batch = None):
    ...
    sendlist_C = [rank_2D[rank][0] * TP_sqrt + rank_2D[rank][1] for rank in range(TP)]
    recvlist_C = [map_2D[rank % TP_sqrt][rank // TP_sqrt] for rank in range(TP)]

    if my_rank == root_rank:
        print("myid " + str(my_rank) + "\nsendlist_B " + str(sendlist) + " recvlist_B " + str(recvlist))
        print("myid " + str(my_rank) + "\nsendlist_C " + str(sendlist_C) + " recvlist_C " + str(recvlist_C))
